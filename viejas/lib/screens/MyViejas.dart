...
import 'package:viejas/model/myviejas.dart';
import 'package:viejas/screens/WebViewScreen.dart';

class MyViejasScreen extends StatefulWidget {
  const MyViejasScreen({Key? key}) : super(key: key);

  @override
  _MyViejasScreenState createState() => _MyViejasScreenState();
}

class _MyViejasScreenState extends State<MyViejasScreen> {
  ...
  Future<dynamic> getDataFromAPI() async {
    ...
    if (response.statusCode == 200) {
      var usersListArray = myViejasObjectFromJson(response.body);
  ...
  FutureBuilder<dynamic> _buildFuture() {
    return FutureBuilder<dynamic>(
      ...
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          if (snapshot.data is List<MyViejasDatum>?) {
            List<MyViejasDatum>? usersArray = snapshot.data;
  ...
  ListView _buildListView(BuildContext context, List<MyViejasDatum> users) {
  ...
  Widget _buildDiningCell(BuildContext context, MyViejasDatum obj) {
